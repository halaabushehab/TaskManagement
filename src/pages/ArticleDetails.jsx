...
import { database, push, ref, onValue, auth, onAuthStateChanged } from '../firebase/firebaseConfig.js';
...
const ArticleDetails = () => {
  ...
  const handleCommentSubmit = (event) => {
    ...
    if (!user) {
      alert('يجب عليك تسجيل الدخول لإضافة تعليق.');
      return;
    }
    ...
    push(ref(database, `comments/${id}`), {
      name: user.displayName || user.email, // استخدام اسم المستخدم أو البريد الإلكتروني
      comment,
        ...
        <div className="col-lg-7">
          <div className="col-lg-12">
            <div className="single-post mb-4" key={id}>
              <div className="feature-img" style={{ position: 'relative', overflow: 'hidden', borderRadius: '15px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.1)' }}>
                <img 
                ...
                <div className="row">
                  <div className="col-12">
                    <div className="form-group mb-4">
                      <textarea className="form-control w-100" name="comment" id="comment" cols="30" rows="3" placeholder="اكتب تعليقك..." required></textarea>
                    </div>
                  </div>
                </div>
                <div className="form-group text-center">
                  <button type="submit" className="btn" style={{ backgroundColor: '#B7A3F7', color: '#fff', padding: '10px 20px' }}>إرسال</button>
                </div>
              </form>
            </div>
          </div>
